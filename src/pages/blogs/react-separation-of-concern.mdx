...
To explain this, I'll use a simple example of `counter`. We have a `Counter` component which has a button to increment the counter and a button to decrement the counter. The `Counter` component is responsible for rendering the UI and handling the click events. The `Counter` component is also responsible for updating the counter value. This is the UI and business logic in one component.
...
```
***Current Example [Link](https://codesandbox.io/s/react-counter-app-776jex?file=/src/Counter.js)***
...
```
***Example with separation of concern [Link](https://codesandbox.io/s/react-counter-app-776jex?file=/src/Counter.js)***
